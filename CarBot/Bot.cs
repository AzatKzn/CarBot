...
namespace CarBot
{
	class Bot
	{
		...
		private void CloseNotFinishedGroupRace()
		{
			using (var context = new AppDbContext())
			{
				...
				if (grouprace != null)
				{
					...
					context.SaveChanges();
					var users = context.GroupRaceParticipant.Where(x => x.GroupRace.Id == grouprace.Id && x.Place != 0).Include(x => x.User).ToList();
					var cost = grouprace.RaceDivision.GetJoinCost();
					users.ForEach(x => x.User.Money += cost);
					context.SaveChanges();
				}
				groupRaceIsOn = false;
